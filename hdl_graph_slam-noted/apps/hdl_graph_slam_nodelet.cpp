...
class HdlGraphSlamNodelet : public nodelet::Nodelet {
 public:
  ...
  virtual void onInit() {
    ...
    optimization_timer = mt_nh.createWallTimer(ros::WallDuration(graph_update_interval),
                                               &HdlGraphSlamNodelet::optimization_timer_callback,
                                               this);
 ...
 private:
  ...
  void cloud_callback(const nav_msgs::OdometryConstPtr &odom_msg, const sensor_msgs::PointCloud2::ConstPtr &cloud_msg) {
    ...
    Eigen::Isometry3d odom = odom2isometry(odom_msg); //每一帧的位姿
    ...
    // 更新odom,keyframe_updater主要用來計算前後兩針點雲的距離，true表示爲關鍵幀
  ...
  bool flush_keyframe_queue() {
    ...
    // std::min<int>返回最小值，吧keyframe_queue中的pose取出来，在位姿图中添加节点
    int num_processed = 0;
    for (int i = 0; i < std::min<int>(keyframe_queue.size(), max_keyframes_per_update); i++) {
      ...
      Eigen::MatrixXd
          information = inf_calclator->calc_information_matrix(keyframe->cloud, prev_keyframe->cloud, relative_pose);
  ...
  bool flush_gps_queue() {
    ...
    auto remove_loc = std::upper_bound(gps_queue.begin(), gps_queue.end(), keyframes.back()->stamp,
                                       [=](const ros::Time &stamp,
                                           const geographic_msgs::GeoPointStampedConstPtr &geopoint) {
  ...
  bool flush_imu_queue() {
    ...
    for (auto &keyframe : keyframes) {
      ...
      if (enable_imu_acceleration) {
        ...
        g2o::OptimizableGraph::Edge *edge = graph_slam->add_se3_prior_vec_edge(keyframe->node,
  ...
  bool flush_floor_queue() {
    ...
    for (const auto &floor_coeffs : floor_coeffs_queue) {
      ...
      Eigen::Vector4d
          coeffs(floor_coeffs->coeffs[0], floor_coeffs->coeffs[1], floor_coeffs->coeffs[2], floor_coeffs->coeffs[3]);
  ...
  void optimization_timer_callback(const ros::WallTimerEvent &event) {
    ...
    // loop detection
    std::vector<Loop::Ptr> loops = loop_detector->detect(keyframes, new_keyframes, *graph_slam); // 会还检测
    for (const auto &loop : loops) {
      Eigen::Isometry3d relpose(loop->relative_pose.cast<double>());
      Eigen::MatrixXd
          information_matrix = inf_calclator->calc_information_matrix(loop->key1->cloud, loop->key2->cloud, relpose);
    ...
    trans_odom2map_mutex.lock();
    trans_odom2map = trans.matrix().cast<float>();  // 当前pose
    trans_odom2map_mutex.unlock();
    ...
    if (odom2map_pub.getNumSubscribers()) {
      geometry_msgs::TransformStamped
          ts = matrix2transform(keyframe->stamp, trans.matrix().cast<float>(), map_frame_id, odom_frame_id);
  ...
  bool dump_service(hdl_graph_slam::DumpGraphRequest &req, hdl_graph_slam::DumpGraphResponse &res) {
    ...
    graph_slam->save(directory + "/graph.xchu_g2o");
    for (int i = 0; i < keyframes.size(); i++) {
