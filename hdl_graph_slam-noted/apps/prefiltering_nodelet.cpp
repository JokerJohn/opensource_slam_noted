...
private:
  void initialize_params() {
    ...
    // 离群点去除的方法，离群点可能是激光雷达测量产生的误差，比如扫描到微小物体表面或者是树木草丛上，可能影响后续点云匹配结果
    // StatisticalOutlierRemoval:基于统计分析的方法，计算输入数据中对点到临近点的距离分布。
    // 对每个点，我们计算它到它的所有临近点的平均距离。假设得到的结果是一个高斯分布，其形状由均值和标准差决定，平均距离在标准范围（由全局距离平均值和方差定义）之外的点，可被定义为离群点并可从数据集中去除掉。
    // RadiusOutlierRemoval:
    //对每个点的临域进行一个统计分析，并修剪掉那些不符合一定标准的点。稀疏离群点移除方法基于在输入数据中对点到临近点的距离分布的计算。
    // 对每个点，我们计算它到它的所有临近点的平均距离。假设得到的结果是一个高斯分布，其形状由均值和标准差决定，平均距离在标准范围（由全局距离平均值和方差定义）之外的点，可被定义为离群点并从数据集中去除掉。
